...
const getCtrlKey = (event: KeyboardEvent): boolean =>
  !!(event.metaKey || event.ctrlKey);

const hasModifierKeys = (event: KeyboardEvent): boolean =>
  event.shiftKey || event.altKey || getCtrlKey(event);

export const isEnter = (event: KeyboardEvent): boolean =>
  (event.key ? event.key === 'Enter' : event.keyCode === KeyCodes.ENTER) &&
  !hasModifierKeys(event);

export const isSpace = (event: KeyboardEvent): boolean =>
  (event.key
...
export const isEscape = (event: KeyboardEvent): boolean =>
  (event.key
    ...
    : event.keyCode === KeyCodes.ESCAPE) && !hasModifierKeys(event);
